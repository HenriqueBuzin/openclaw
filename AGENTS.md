...
- O backend entra diretamente na `proxy-network`; o proxy global da VPS termina TLS e encaminha
  HTTP/WebSocket para `openclaw:18789` ou `openclaw-dev:18789`.
...
Configuráveis: `OPENCLAW_LOCAL_PORT`, `OPENCLAW_DATA_ROOT`, `OLLAMA_DATA_ROOT`, `OLLAMA_MODEL`,
`OLLAMA_CONTEXT_LENGTH`, `OLLAMA_KEEP_ALIVE`, `IMAGE_TAG`, `BUILD_COMMIT`, `BUILD_DATE`,
`BUILD_NUMBER`. Somente o Compose dev publica `127.0.0.1:${OPENCLAW_LOCAL_PORT}:18789`; produção
...
Para reconstruir do zero, recrie exatamente os arquivos acima, preserve os nomes de serviços,
redes e variáveis, gere um token de gateway, crie a rede externa `proxy-network`, disponibilize os
diretórios persistentes e execute o Compose do ambiente. Configure o proxy global para o alias do
ambiente na porta `18789`. Não adicione proxy interno, banco de dados, Redis, profiles, Nginx ou
comandos de aplicação aos Compose.
